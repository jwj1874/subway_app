...
class _SeatReservationScreenState extends State<SeatReservationScreen> {
  ...
  void initState() {
    ...
    _initialization = _initSeatsOnDemand().then((_) {
      // After initialization, start listening for occupancy changes
  ...
  Future<void> _initSeatsOnDemand() async {
    ...
    if (doc.exists) return; // Seats already created

    final batch = _db.batch();
    batch.set(_carDocRef, {'createdAt': FieldValue.serverTimestamp()});

    for (int i = 1; i <= 12; i++) {
      final ref = _seatsCollectionRef.doc(i.toString()); // Use seat number as doc ID
      batch.set(ref, {
        'seatNumber': i,
        'isPriority': (i == 1 || i == 6), // Original priority seat logic
        'reserved': false,
        'reservedBy': null,
        'isOccupied': false, // Add the new field
